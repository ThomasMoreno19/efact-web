...
class PantallaCliente {
    ...
    async mostrarTodo() {
        ...
        try {
            ...
            rubrosRecibidos.forEach(rubro => {
                ...
                // Agregar el evento de clic al botón de rubro
                rubroBoton.addEventListener('click', () => {
                    this.barraBusqueda.value = '';
                    this.tituloRubros.classList.add('hidden');
    ...
    filtrarArticulosPorRubro(idRubroSeleccionado) {
        ...
        // Mostrar solo el contenedor del rubro seleccionado
        const rubroSeleccionado = document.querySelector(`.container-rubro[data-rubro-id="${idRubroSeleccionado}"]`);
        if (rubroSeleccionado) {
            rubroSeleccionado.classList.remove('hidden');
            this.listaArticulos.classList.remove('hidden');
        }
    ...
    filtrarArticulosEnRubro(idRubroSeleccionado) {
        ...
        const listaPlana = this.crearListaPlanaDesdeArticulos(articulosFiltrados, 'No se encontraron artículos en este rubro.');
    ...
    botonTelefono() {
        ...
        telefono.addEventListener('click', () => {
            navigator.clipboard.writeText(this.empresa.telefono)
                .then(() => {
                    const numeroWhatsApp = this.empresa.telefono.replace(/[^0-9]/g, ''); // limpia espacios o símbolos
