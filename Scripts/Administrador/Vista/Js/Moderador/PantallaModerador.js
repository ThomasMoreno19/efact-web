...
class PantallaModerador {
    ...
    async abrirModalModificar(modalPadre) {
        ...
        const modal = this.empresa.modalModificar(moderador);
        ...
        modal.addEventListener('click', (event) => {
            ...
            try {
                ...
                const empresaModificada = this.gestor.modificarEmpresa(this.empresa.id, nombre, telefono, ubicacion);
                modal.classList.add('hidden');
                document.body.removeChild(modal);
    ...
    async configurarEmpresa() {
        ...
        const idEmpresa =document.getElementById('id-empresa-modal');
        idEmpresa.classList.add('hidden');
        botonGestionArticulos.classList.add('hidden');
    ...
    async abrirModalCambiarLogo(modalPadre) {
        ...
        form.addEventListener('submit', async (event) => {
            ...
            try {
                const empresaModificada = await this.gestor.cambiarLogoEmpresa(this.empresa.id, imagen, this.empresa.nombre);
                modal.classList.add('hidden');
                document.body.removeChild(modal);
                await this.mostrarLista();
