class ProveedorVista {
  constructor(proveedor) {
    const { id, id_empresa, nombre, logo_url, video_url = null } = proveedor;
    this.id = id;
  ...
  modalConfigurar() {
    ...
    modal.id = "modal-configurar-proveedor";
    ...
    <form id="form-configurar-proveedor">
      <h2 id="nombre-articulo-modal">${this.nombre}</h2>
      <button type="button" class="submit-button" id="modificar">Modificar</button>
      <button type="button" class="submit-button" id="boton-subir-video-proveedor">Subir video/imagen</button>
    </form>`;
  ...
  mostrarModalReproductor(url) {
    const modal = document.createElement("div");
    modal.classList.add("modal");
    modal.id = "modal-video-proveedor";
  ...
  modalEliminarVideo() {
    ...
    confirmarBtn.addEventListener("click", () => {
      const event = new CustomEvent("videoEliminarProveedor", { detail: this });
      document.dispatchEvent(event);
      if (typeof window.eliminarVideoProveedor === "function") {
        window.eliminarVideoProveedor(this);
        document.getElementById("modal-video-proveedor").remove();
        modal.remove();
      }
    });
