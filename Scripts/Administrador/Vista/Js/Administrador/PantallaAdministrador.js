class PantallaAdministrador {
  ...
  async modalEmpresaSeleccionada(empresa) {
    ...
    BOTON_ELIMINAR_EMPRESA.addEventListener("click", async (event) => {
      event.preventDefault();
      const CONFIRMACION = confirm(
        "¿Estás seguro de que deseas eliminar esta empresa? Esta acción no se puede deshacer.",
      );
      if (CONFIRMACION) {
        await this.gestor.eliminarEmpresa(empresa.id);
        document.body.removeChild(MODAL);
        this.mostrarLista();
      }
    });
  ...
  abrirModalNuevaEmpresa() {
    ...
    FORM.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        const EMPRESA = await this.gestor.crearEmpresa(
          NOMBRE,
          TELEFONO,
          UBICACION,
          TIENE_CARRITO,
