...
class GestorAdministrador {
  async mostrarListaEmpresas() {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Error al obtener empresas: ${response.status} - ${errorData.error || errorData.message}`,
        );
      }
  ...
  async crearEmpresa(
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ message: "Error desconocido al crear empresa" }));
        throw new Error(
          errorData.error || `Error al crear la empresa: ${response.status}`,
        );
      }
  ...
  async crearModerador(nombre, id_empresa, contrasena) {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ message: "Error desconocido al crear moderador" }));
        throw new Error(
          errorData.message ||
  ...
  async obtenerLogoEmpresa(id_empresa) {
    try {
      ...
      console.error(
        "Error al obtener el logo de la empresa ",
        id_empresa,
        ":",
        error,
      );
  ...
  async eliminarEmpresa(id_empresa) {
    try {
      const bodyData = { id_empresa: id_empresa };
      const response = await fetch(`/empresa/eliminar`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(bodyData),
      });
      ...
      console.error("Error al eliminar la empresa ", id_empresa, ":", error);
      throw error;
    }
  }
}
