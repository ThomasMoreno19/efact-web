...
require_once $_SERVER['DOCUMENT_ROOT'] . '/Scripts/Administrador/Modelo/Repositorio/InternoRepositorio.php';
...
class GestorInterno
{
  ...
  {
    $this->pdo = $pdo;
    $this->repositorio = new RepositorioInterno($pdo);
  }
  ...
  private function login(): void
  {
    ...
    if (!isset($_SESSION['loginInterno'][$idEmpresa])) {
      $_SESSION['loginInterno'][$idEmpresa] = [
        'intentos' => 0,
        'bloqueadoHasta' => null
      ];
    }

    $estado = &$_SESSION['loginInterno'][$idEmpresa];
    ...
    $interno = $this->repositorio->obtenerPorEmpresa($idEmpresa);
    ...
    if (!password_verify($password, $interno['contrasena'])) {
