...
class ProveedorRepositorio
{
  ...
  {
    $stmt = $this->pdo->prepare('SELECT 1 FROM proveedor WHERE id = :id AND id_empresa = :id_empresa LIMIT 1');
  ...
  {
    $stmt = $this->pdo->prepare('UPDATE proveedor SET logo_url = :logo_url WHERE id = :id AND id_empresa = :id_empresa');
